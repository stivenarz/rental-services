...
export default function Navbar({ brand = 'MiMarca', links = null, user, setSearch }) {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [profileOpen, setProfileOpen] = useState(false);
  const profileRef = useRef(null);
  const InputSearch$ = new BehaviorSubject('')
    ...
    <header className="nm-header">
      ...
      <nav className="nm-nav" aria-label="Main navigation">
        <div style={{display:'flex', alignItems:'center', gap:12}}>
          <h2 className="nm-brand" aria-label="Ir al inicio">
            <div className="nm-logo">{brand[0] ?? 'M'}</div>
            <h1 style={{fontSize:16}}>{brand}</h1>
          </h2>

          {/* Desktop links */}
          <div className="nm-links" role="menubar">
            {navLinks.map((l) => (
              <a key={l.label} href={l.href} className="nm-link" role="menuitem">{l.label}</a>
            ))}
          </div>
        </div>
        ...
        <div style={{display:'flex', alignItems:'center', gap:12}}>
          <div className="nm-search" role="search" aria-label="Buscar">
            ...
            <input type='search' placeholder="Buscar..." aria-label="Buscar" onChange={(e)=>{setSearch(e.target.value)}} />
          </div>
          ...
          <div className="nm-actions">
            ...
            {!user ? 
              <a href="/login" className="nm-dropdown-item">Iniciar sesión</a>
             : 
            <div className="nm-dropdown" ref={profileRef}>
              <div style={{display:'flex', alignItems:'center', gap:8}}>
                ...
                >
                  <span style={{fontWeight:700}}>SA</span>
                </button>
              </div>
              ...
              {profileOpen && (
                <div className="nm-dropdown-panel" role="menu" aria-label="Perfil">
                  ...
                  <a className="nm-dropdown-item" onClick={e=>logOut()} >Cerrar sesión</a>
                </div>
              )}
            </div>
            }
      ...
      {mobileOpen && (
        <div className="nm-mobile-menu" role="dialog" aria-label="Menú móvil">
          <div className="nm-mobile-panel">
            {navLinks.map((l) => (
              <a key={l.label} href={l.href} className="nm-mobile-link" onClick={() => setMobileOpen(false)}>{l.label}</a>
            ))}
